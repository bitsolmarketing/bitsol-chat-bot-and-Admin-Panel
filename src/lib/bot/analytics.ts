...
export async function chatbotAnalytics(days: number): Promise<QueryResult<ChatbotAnalytics>> {
  ...
  return safeQuery(async () => {
    ...
    const [
      ...
      prisma.marketingLead.count({ where: whatsappLeads }),
      prisma.marketingLead.count({ where: { ...whatsappLeads, temperature: { in: ["WARM", "HOT", "HIGH_PRIORITY"] } } }),
      prisma.marketingLead.count({ where: { ...whatsappLeads, temperature: { in: ["HOT", "HIGH_PRIORITY"] } } }),
      prisma.marketingLead.count({ where: { ...whatsappLeads, stage: "WON" } }),
      prisma.marketingLead.count({ where: { source: "WHATSAPP", stage: "LOST", updatedAt: { gte: since } } }),
      ...
      prisma.marketingLead.groupBy({ by: ["temperature"], _count: { _all: true }, where: whatsappLeads }),
      prisma.marketingLead.groupBy({ by: ["stage"], _count: { _all: true }, where: whatsappLeads }),
      prisma.marketingLead.groupBy({ by: ["subService"], _count: { _all: true }, where: whatsappLeads }),
      prisma.marketingLead.groupBy({ by: ["intent"], _count: { _all: true }, where: whatsappLeads }),
      prisma.marketingLead.groupBy({ by: ["country"], _count: { _all: true }, where: whatsappLeads }),
