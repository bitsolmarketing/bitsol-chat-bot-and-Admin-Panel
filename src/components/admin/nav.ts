...
export const ADMIN_NAV: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Meetings", href: "/admin/meetings", icon: CalendarDays, permission: "meetings.manage" },
    ],
  },
  {
    label: "Practice",
...
/** Filter navigation for a session: items are hidden when the role lacks the permission. */
export function visibleNav(permissions: Set<string> | null): NavGroup[] {
  return ADMIN_NAV.map((group) => ({
    label: group.label,
    items: group.items.filter(
      (item) => !permissions || !item.permission || permissions.has(item.permission)
    ),
