...
import type { ReplyRequest } from "./engine";
import { promptContextFor } from "./prompt";
import type { BotConfig } from "./schema";

/**
 * The model calls behind a `BotRuntime`, shared by the WhatsApp runtime and the
 * console simulator so the simulator answers exactly as a customer would be
 * answered. None of these throw: a model outage becomes an empty string, which
 * the engine turns into an apology and a way to reach the team.
 */
...
export async function representativeReply(ctx: AiContext, request: ReplyRequest): Promise<string> {
  try {
    const plan = planAssistantTurn(ctx.history, {
      channel: "WHATSAPP",
      details: request.details,
      whatsapp: { number: ctx.phone, profileName: ctx.profileName },
      bot: promptContextFor(ctx.config, ctx.language, {
...
export function customerDetailsFrom(ctx: AiContext, known: CustomerDetails): Promise<CustomerDetails> {
  return extractCustomerDetails(ctx.history, known, {
    channelPhone: ctx.phone,
    brandPhones: [ctx.config.contact.whatsappCta, ctx.config.contact.businessPhone],
  });
}
...
export async function translateCopy(ctx: AiContext, text: string, language: Language): Promise<string> {
  try {
    return await collect(
      `Translate the WhatsApp message you are given for a customer of ${ctx.config.personality.assistantName}. ${LANGUAGE_PROFILES[language].promptDirective} Translate faithfully: add nothing, remove nothing. Keep *bold* markers, bullet characters, line breaks, emojis, URLs and product names (WhatsApp, WhatBot Pro, SEO, AI, CRM, n8n) exactly as they are. Reply with the translation only.`,
      text,
      1600,
...
export async function handoverBriefing(ctx: AiContext): Promise<string> {
  const transcript = ctx.history
    .slice(-30)
    .map((turn) => `${turn.role === "user" ? "CUSTOMER" : "BITSOL"}: ${turn.content.slice(0, 800)}`)
    .join("\n");
  try {
    const summary = await collect(
      "You brief a colleague taking over a WhatsApp conversation. In three to five plain English sentences say what the customer wants, the details they gave, what they are waiting for and anything that needs care. Use only what is in the transcript. No headings, no bullet points.",
      transcript,
      500,
      appConfig.ai.extractionModel
    );
